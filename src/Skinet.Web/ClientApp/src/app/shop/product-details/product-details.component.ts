  ...
  loadProduct() {
    ...
    if (id) this.shopSrv.getProduct(+id).subscribe({
      next: prod => {
        ...
        this.basketSrv.basketSource$.pipe(take(1)).subscribe({
          next: basket => {
  ...
  updateBasket() {
    if (this.product) {
      if (this.quantity > this.quantityInBasket) {
        ...
        this.basketSrv.addItemToBasket(this.product, itemsToAdd);
      } else {
        const itemsToRemove = this.quantityInBasket - this.quantity;
        this.quantityInBasket -= itemsToRemove;
        this.basketSrv.removeItemFromBasket(this.product.id, itemsToRemove);
      }
    }
  }
